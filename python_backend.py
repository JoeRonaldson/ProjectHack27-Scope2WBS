...
def extract_text_from_upload(file_name: str, payload: bytes) -> str:
    ...
    raise HTTPException(status_code=400, detail="Unsupported file type. Use TXT, MD, CSV, JSON, DOCX, or PDF.")
...
@app.post("/generate-wbs-from-upload")
async def generate_wbs_from_upload(document: UploadFile = File(...)) -> dict[str, Any]:
    payload = await document.read()
    scope_text = extract_text_from_upload(document.filename or "uploaded-document", payload)
    if not scope_text:
